...
- `bc` installed (for shell scripts)
...
|------|---------|---------|
| **EVM Mainnet Fork** | Ethereum mainnet (real USDC) | `bash script/demo.sh` |
| **XRPL EVM Sidechain** | XRPL EVM testnet (XRP gas) | `bash script/demo-xrpl.sh` |
...
# Replace YOUR_KEY with your Alchemy key
...
**Demo flow:** Same as above, but uses a mock USDC (since USDC doesn't exist on XRPL EVM).
...
- **XRPL Commons** — XRPL EVM sidechain support and demo
